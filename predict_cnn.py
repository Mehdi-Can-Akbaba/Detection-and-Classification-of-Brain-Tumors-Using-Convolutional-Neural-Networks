...
model.load_state_dict(torch.load('cnn/last_model.pth'))  # Eğitilmiş modeli yükle
model.eval()
...
for class_name in class_names:
    ...
    for image_name in selected_images:
        ...
        try:
            font = ImageFont.truetype("arial.ttf", 14)  # Yazı boyutunu 14 olarak ayarladım
        except IOError:
            font = ImageFont.load_default()

        text = f"Predicted class: {predicted_class}, Confidence: {confidence_percentage:.2f}%"
        text_position = (10, 10)
        text_color = "yellow"
        text_thickness = 1
        draw.text(text_position, text, fill=text_color, font=font, stroke_width=text_thickness)

        # Görseli kaydet
        output_image_path = os.path.join(output_dir, f"{class_name}_{os.path.basename(image_path)}")
